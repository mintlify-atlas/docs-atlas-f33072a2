...
# Optional: Overpass API (uses public endpoint by default)
...
<Accordion title="Environment variable reference">
  ...
  |----------|----------|-------------|----------|
  | `DATABASE_URL` | Yes | Database connection string | `file:./db/custom.db` |
  | `NEXT_PUBLIC_APP_URL` | No | Application base URL | `http://localhost:3000` |
  | `NODE_ENV` | No | Environment mode | `development` |
  | `OVERPASS_API_URL` | No | OpenStreetMap Overpass API endpoint | `https://overpass-api.de/api/interpreter` |
</Accordion>
